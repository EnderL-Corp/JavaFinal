...
public class Game extends GameClient implements Serializable {
	...
	/**
	 * Constructor for a connection between multiple computers, with no ties to the actual game.
	 * 
	...
	/**
	 * actionPerformed() for this class as it listens upon a timer, which is declared in GameClient, Game's superclass.
	 * Will update board or send changes in the board to the server when needed.
	 */
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (remoteServer.getConnections() > 1)
				System.out.println(remoteServer.getOtherClient(clientInfo).getTag());			//RMI TESTER IF DELETE PLEASE
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	}
	...
	/**
	 * Every timer tick, the recent board, current player turn, and info of the other client are updated.
	 * Additionally, the program checks for a win or a loss
	 */
	...
	/**
	 * Places an Entity on the board as per the Entity's stored x and y coordinates.
	...
	/**
	 * Will either take or add the specified amplifier based on the boolean take.
	...
	/**
	 * Will be called frequently and do nothing until there is a full selection of actions to act upon. 
	 */
