...
public class Game extends GameClient implements Serializable {
	 ...
	 * 
	 * @param deckEnum
	 *            the DeckEnum of this Game (the player's class)
	 */
		...
		for (int i = 0; i < recentBoard.length; i++) {
			for (int j = 0; j < recentBoard.length; j++) {
				...
				if ((i == 0 || i == recentBoard.length - 1 || j == 0 || j == recentBoard.length - 1)
						|| ((i == 1 || i == recentBoard.length - 2) && (j == 1 || j == 2 || j == 3 || j == 4
								|| j == recentBoard.length - 2 || j == recentBoard.length - 3
								|| j == recentBoard.length - 4 || j == recentBoard.length - 5))
						...
						|| ((i == 5 || i == recentBoard.length - 6)
								&& (j == 6 || j == 7 || j == recentBoard.length - 7))
						|| ((i == 6 || i == 7 || i == recentBoard.length - 7) && (j == 5 || j == 6 || j == 7
								|| j == recentBoard.length - 7 || j == recentBoard.length - 6))) {
	 ...
	 * 
	 * @param card
	 *            the card to place in the graveyard
	 */
	 ...
	 * 
	 * @param serverIP
	 *            the IP of the server on which the server will run.
	 */
	public static void createHost(String serverIP) {
		...
		try {
			...
			game.clientInfo = new ClientInfo(game.getName(), game.getCommander(), game.getTag(), game.cp, game.ap,
					game.tp);
	 ...
	 * 
	 * @param serverIP
	 *            the IP at which the server is running
	 */
	...
	/**
	 * Changes phase. -1 is opponents turn, 1 and 2 are the Player's turn and 2
	 * ends the turn
	 */
	...
	public void endTurn() {
		try {
			...
			for (int i = 0; i < 5; i++) {
				...
				} else if (found.contains(ampPanel[i]) && (ampPanel[i].getAmpType() == AmpEnum.HEALING_WAVE
						|| ampPanel[i].getAmpType() == AmpEnum.SHORT_CIRCUIT)) {
	 ...
	 * 
	 * @return the recent information on the other client, updated every timer
	 *         event
	 */
	 ...
	 * 
	 * @param winOrLose
	 *            the boolean representing a win or a loss
	 */
	 ...
	 * 
	 * @param e
	 *            the Entity to place on the board
	 ...
	 * 
	 * @param slot
	 *            the slot to look in for the Amplifier
	...
	public boolean updateAmpPanel(Amplifier amp, boolean take) {
		...
		if (take) {
			for (int i = 0; i < ampPanel.length; i++) {
				if (game.getAmpAt(i).getName() == this.getName() && updated == false) // XXX
																						// Replace
																						// "this"
																						// with
																						// "amp"?
	 ...
	 * 
	 * @param c
	 *            the Card to add to the action queue
	 */
	...
	/**
	 * <code>executePlayerActionQueue()</code> is responsible for the movement,
	 * attacking, and placing of Entities, as well as the placement of Gear,
	 * Amplifiers, and Techniques. The method gets the action the player wants
	 * to execute from <code>currentPlayerAction</code> and then checks to see
	 * if the selections made for that actions are appropriate. If they are, the
	 * correct method is run to do the action of the Card.
	 */
		...
		if (myTurn) {
			...
			case 'M': // MOVE
				if (phase == 1 && first instanceof Entity && second instanceof MovePoint) {
					...
					CommandLog.publish(
							"[Game] You are moving " + first.getName() + " to position " + ((MovePoint) second) + ".");
					try {
						remoteServer.setRecentClientActionDescription("[Game] Opponent moved Entity" + first.getName()
								+ " to position " + ((MovePoint) second) + ".");
					} catch (RemoteException e) {
						e.printStackTrace();
					}
					boardChanged = true;
			...
			case 'A': // ATTACK
				if (phase == 1 && first instanceof Entity && second instanceof Entity)
					if (!(((Entity) first).hasAbility(3) && second instanceof Commander)) {
						...
						CommandLog.publish("[Game] You, Entity " + first.getName() + ", are attacking Entity "
								+ second.getName() + ".");
						try {
							remoteServer.setRecentClientActionDescription("[Game] Opponent, Entity " + first.getName()
									+ ", attacked Entity " + second.getName() + ".");
						} catch (RemoteException e) {
							e.printStackTrace();
						}
						boardChanged = true;
							...
							for (int i = 1; i < queuedPlayerActions.size(); i++)
								if (queuedPlayerActions.get(i) instanceof Troop) {
									...
									if (queuedPlayerActions.get(i) instanceof Troop) {
										...
										try {
											remoteServer
													.setRecentClientActionDescription("[Game] Opponent used technique "
															+ first.getName() + " on Troop " + second.getName() + ".");
										} catch (RemoteException e) {
											e.printStackTrace();
										}
									}
			...
			case 'G':
				if (phase == 0 && first instanceof Gear && second instanceof Troop) {
					...
					CommandLog.publish(
							"[Game] You are using the Gear " + first.getName() + " on Troop " + second.getName() + ".");
					try {
						remoteServer.setRecentClientActionDescription("[Game] Opponent used the Gear " + first.getName()
								+ " on Troop " + second.getName() + ".");
					} catch (RemoteException e) {
						e.printStackTrace();
					}
					...
					if ((getColor() == Color.CYAN && ((MovePoint) second).getX() <= territory)
						...
						if (cp >= ((Troop) first).getCpCost()) {
							CommandLog.publish("[Game] You are placing the Troop " + first.getName() + " on Position "
									+ ((MovePoint) second) + ".");
							try {
								remoteServer.setRecentClientActionDescription("[Game] Opponent placed the Troop "
										+ first.getName() + " at Position " + ((MovePoint) second) + ".");
							} catch (RemoteException e) {
								e.printStackTrace();
							}
			...
			case 'S':
				if (phase == 0 && first instanceof Amplifier && myHand.contains(first)
						&& !(((Amplifier) first).getAmpType().equals(AmpEnum.NONE)) && second instanceof Amplifier
					...
					if (ampPanel[i] == null) {
						...
						try {
							remoteServer.setRecentClientActionDescription(
									"[Game] Opponent used the Amplifier " + first.getName() + ".");
						} catch (RemoteException e) {
							e.printStackTrace();
						}
						boardChanged = true;
