...
public class Troop extends Entity {
	protected boolean[] abilities; // in this order: {provoke, deflect, blast,
									// range, mirror, void}
	...
	/**
	 * Creates a troop, the most common unit in the game, placed onto the board
	...
	/**
	 * @return ImageIcon - The path to both this Troop and its overlaying troop
	 *         type.
	 */
	...
	/**
	 * @param abs
	 *            - The index of the ability (i.e. 1 for deflect, or 5 for void
	 *            [see above])
	 * @return boolean - Whether or not the ability was added
	 */
	public boolean addAbilities(int abs) {
		if (abs == 1)
			deflectTime = true; // in order to reset the abilities each turn,
								// you just have to addAbilites(1 or 4) at the
								// beginning of the phase
		if (abs == 4)
			mirrorTime = true;

		if (abilities[abs] == false) {
			if ((abilities[2] == true || abilities[3] == true) && (abs == 2 || abs == 3)) // makes
																							// sure
																							// you
																							// dont
																							// have
	...
	/**
	 * <code>canDeflect()</code> determines whether or not a Troop can deflect
	 * an attack. A troop can only deflect once per turn, so if the troop can
	 * deflect, it will, and its ability to deflect is set to false until next
	 * turn
	 * 
	...
	/**
	 * <code>canMirror()</code> determines whether or not a Troop can mirror an
	 * attack. A troop can only mirror once per turn, so if the troop can
	 * mirror, it will, and its ability to mirror is set to false until next
	 * turn
	 * 
	...
	/**
	 * @param int
	 *            - the ability to check for (i.e. 1 for deflect, or 5 for void
	 *            [see above])
	...
	/**
	 * <code>attack(defender)</code> attempts to make the Troop attack the
	 * defender. The method checks to see if the defender is at most 1 tile away
	 * (unless the attacker has range) and if any troop adjacent to the attacker
	 * has provoke. If the attack is valid, this method then calls
	 * <code>dealDamage(defender)</code> or <code>blastAttack(defender)</code>
	 * if the attacker has blast.
	 * 
	 * @param Entity
	...
	/**
	 * <code> kill(killed) </code> moves a specified "dead" entity to the
	 * graveyard. The code then moves any gear that troop possessed to the
	 * graveyard as well.
	 * 
	 * @param Entity
	 *            - The killed Entity
	 ...
	 *            - The Troop to place
	 * @param int
	 *            - The amount of Creature Points the Player has
	...
	/**
	 * <code> blastAttack(defender) </code> performs a "blast attack". First, it
	 * looks for where the defender is in relation to itself. If the defender is
	 * at a diagonal, it can't blast attack diagonally, and so will either
	 * attack normally if defender is in range, or not attack at all if its not.
	 * For the actual blast attack, it looks at whether the defender is in a
	 * different row or a different column as the attacker. Then, It does damage
	 * to everything in that row/column (including those which are on the
	 * attackers side) starting from the attacker (but not including it), and
	 * going until it hits the end of the array or a void troop.
	 * 
	 * @param defender
	 */
	public void blastAttack(Entity defender) {
		...
		if (defender.getPosY() < yCoordinate)
			colChange = -1;
		// checking to see where in relation the defender is too attacker
		// (direction wise)

		if (rowChange != 0 && colChange != 0) // this means that the defender is
												// diagonal, and you cant
												// blastAttack diagonal, so
		...
		if (rowChange == 1 || rowChange == -1) {
			for (int row = xCoordinate; row < 15 && row > -1; row += rowChange) {
				if (Game.game.getEntityAt(row, yCoordinate) != null
						&& Game.game.getEntityAt(row, yCoordinate) != this) {
			...
			for (int col = yCoordinate; col < 15 && col > -1; col += colChange) {
				if (Game.game.getEntityAt(xCoordinate, col) != null
						&& Game.game.getEntityAt(xCoordinate, col) != this) {
